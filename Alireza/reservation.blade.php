...
@section('scripts')
    ...
    // Instance-level settings old
        ...
        @if ($default_reservation_type == 'future')
            <?php $is_online_reservation = 1 ?>
        @elseif ($default_reservation_type == 'immediate') 
            <?php $is_online_reservation = 0 ?>
        @endif
        ...
        var is_admin = true;
        var is_online_reservation = {{ $is_online_reservation }};
        ...
        // var stripe_api_key = "{{ App\Setting::get('services_stripe_public_key') }}"; Alireza : testMode should be considered
        var stripe_api_key = "{{ $testmode ? $stripe_testkey : $stripe_key }}";
        var CardconnectApiHost = "{{ config('services.cardconnect.apiHost') }}:{{ config('services.cardconnect.apiPort') }}";
    ...
    // Rental type-level settings
        ...
        var show_category_image_to_customer = {{ App\RentalTypeSetting::get($rental_type->id, ($is_online_reservation ? 'show_category_image_to_online_customer' : 'show_category_image_to_walkin_customer')) ? App\RentalTypeSetting::get($rental_type->id, ($is_online_reservation ? 'show_category_image_to_online_customer' : 'show_category_image_to_online_customer')) : 0 }};
        ...
        var delivery_message = "{!! App\RentalTypeSetting::get($rental_type->id, 'delivery_message') !!}";
    ...
    <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCmh_CMTBdI9bRBbRfDFv0jNpXo0BKME6Y"
